...
metadata:
  { "slashbot": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${OPENCLAW_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/slashbot-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/slashbot.sock"
SESSION=slashbot-python
...
- Default socket path: `"$OPENCLAW_TMUX_SOCKET_DIR/slashbot.sock"`.
