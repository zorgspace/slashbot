...
export async function executeHeartbeat(
  ...
  step.heartbeat(action.prompt ? 'custom prompt' : 'reflection');
  ...
  try {
    ...
    step.heartbeatResult(result.type === 'ok');
...
export async function executeHeartbeatUpdate(
  ...
  try {
    ...
    step.heartbeatUpdateResult(success);
    ...
    const errorMsg = error?.message || String(error);
    step.heartbeatUpdateResult(false);
