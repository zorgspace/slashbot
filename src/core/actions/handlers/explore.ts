...
import type { ActionResult, ActionHandlers, ExploreAction, GrepOptions } from '../types';
...
export async function executeExplore(
  ...
  step.tool('Explore', `"${query}" in ${paths.join(', ')} (${depth})`);
  ...
  try {
    ...
    step.error(`Explore failed: ${errorMsg}`);
    return {
      action: `Explore: ${query}`,
      success: false,
      result: 'Failed',
