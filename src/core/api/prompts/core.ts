...
export const CORE_PROMPT = [
  ...
  '- NEVER declare work finished if `<format>` or `<exec>` returned errors. Fix all errors before ending. Run `<format>` after edits to verify syntax.',
  '',
  '# Code Quality',
  ...
  '- After every `<edit>`, run `<format path="file"/>` to verify. If it reports errors, fix immediately — do NOT proceed or `<end>` with syntax errors.',
