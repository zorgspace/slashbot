...
export class CommandPermissions {
  ...
  async promptForApproval(command: string, folder: string): Promise<PromptResult> {
    return new Promise(resolve => {
      ...
      const options: { label: string; value: PromptResult }[] = [
        { label: 'Allow once', value: 'yes' },
        { label: `Always allow '${cmdBase}' in ${folderName}`, value: 'always' },
        { label: 'Deny', value: 'no' },
      ];
      ...
      const renderMenu = () => {
        ...
        options.forEach((opt, i) => {
          if (i === selectedIndex) {
            display.violet(`› ${opt.label}`);
          } else {
            display.muted(`  ${opt.label}`);
          }
        });
        display.muted('  ↑↓ select · Enter confirm');
      };

      // Initial render
      display.append('');
      display.warningText('Command execution requested:');
      display.violet(`  $ ${command}`);
      display.muted(`  in ${folder}`);
      display.append('');
