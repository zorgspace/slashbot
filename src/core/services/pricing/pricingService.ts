...
export class PricingService {
  ...
  /**
   * Get the price multiplier (e.g., 5x)
   */
  ...
  async calculateCost(
    ...
    // Apply multiplier (5x)
    const multipliedUsdCost = baseUsdCost * this.config.multiplier;

    // Convert to SOL
    const solCost = usdToSol(multipliedUsdCost, exchangeRates.solUsd);
  ...
  calculateCostSync(
    ...
    const baseUsdCost = calculateBaseUsdCost(model, inputTokens, outputTokens);
    const multipliedUsdCost = baseUsdCost * this.config.multiplier;
    const solCost = usdToSol(multipliedUsdCost, rates.solUsd);
    const slashbotCost = usdToSlashbot(
      multipliedUsdCost,
      rates.solUsd,
      rates.slashbotSol
  ...
  async getPricingInfo(model: string): Promise<{
    ...
    const inputUsd = pricing.inputPricePerMillion * this.config.multiplier;
    const outputUsd = pricing.outputPricePerMillion * this.config.multiplier;
