...
import { encryptPrivateKey, decryptPrivateKey, verifyPassword, createSignatureMessage, signMessage, hashData } from './crypto';
import {
  generateKeypair,
  importKeypair,
  getSolBalance,
 ...
 */
export function createWallet(password: string): { publicKey: string; seedPhrase: string; wallet: EncryptedWallet } {
 ...
 */
export function importWallet(privateKeyBase58: string, password: string): { publicKey: string; wallet: EncryptedWallet } {
