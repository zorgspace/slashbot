...
export function getBashToolContributions(): ToolContribution[] {
  return [
    {
      name: 'bash',
      description: 'Execute a shell command. Returns stdout/stderr. Use for git, npm, build tools, and other CLI operations.',
      parameters: z.object({
        ...
        background: z.boolean().optional().describe('Run in background without waiting for completion'),
      }),
      toAction: (args) => ({
        type: 'bash',
