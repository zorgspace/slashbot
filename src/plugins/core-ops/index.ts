...
const COMMAND_GUIDE: Record<string, { usage: string; when: string }> = {
  ...
  plugins: {
    usage: '/plugins',
    when: 'Use when you need to verify which plugins are currently loaded.',
  },
  update: {
    ...
    setup: (context) => {
      ...
      context.registerCommand({
        ...
        execute: async (_args, commandContext) => {
          ...
          if (!loadedPlugins) {
            commandContext.stdout.write('Kernel not available\n');
            return 1;
          }

          const plugins = loadedPlugins();
          commandContext.stdout.write('Loaded plugins:\n');
          for (const id of plugins) {
            commandContext.stdout.write(`  ${id}\n`);
          }
