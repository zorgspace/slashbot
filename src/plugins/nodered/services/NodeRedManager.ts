...
import type { IndicatorStatus, PathResolver, StructuredLogger } from '../../../core/kernel/contracts.js';
import type { NodeRedState, NodeRedConfig, NodeRedStatus, NodeRedRuntimeState } from '../types.js';
...
export class NodeRedManager {
  ...
  constructor(events: EventBus | undefined, logger: StructuredLogger, paths: PathResolver) {
  ...
  private getDefaultConfig(): NodeRedConfig {
    return {
      enabled: true,
      port: 1880,
      userDir: this.paths.home('nodered'),
   ...
   * Publishes status event and updates indicator on state changes.
   */
  private setState(newState: NodeRedState): void {
    ...
    // Map state to indicator status
    const indicatorMap: Record<NodeRedState, IndicatorStatus> = {
      disabled: 'off',
    ...
    this.indicatorUpdater?.(indicatorMap[newState]);
  }

  /**
   * Publish a Node-RED status event via EventBus.
   */
  private publishStatus(status: string, extra?: Record<string, unknown>): void {
    this.eventBus?.publish('nodered:status', { status, ...extra } as Record<string, string | number>);
  }
  ...
  async init(): Promise<void> {
    // Idempotency guard
    if (this.runtimeState.state !== 'disabled') return;

    const defaults = this.getDefaultConfig();

    // Load config from file
    const configPath = this.paths.home('nodered.json');
    try {
    ...
    if (exitCode !== 0) {
      ...
      this.publishStatus('failed', { error: 'Node.js not found. Please install Node.js to use Node-RED.' });
      return;
    }
    ...
    if (!installResult.success) {
      ...
      this.publishStatus('failed', { error: installResult.error || 'Failed to install Node-RED' });
      return;
    }
    ...
    try {
      ...
      if (response.ok && response.status === 200) {
        // Stale process detected - adopt it
        this.setState('starting');
        this.setState('running');
        this.runtimeState.startedAt = new Date();
        this.startHealthCheckTimer();
        this.publishStatus('running', { port: this.config.port });
        return;
      }
    } catch {
  ...
  private async ensureNodeRedInstalled(userDir: string): Promise<{ success: boolean; error?: string }> {
    ...
    this.logger.info('Installing Node-RED', { userDir });
    ...
    try {
      ...
      this.logger.info('Node-RED installed successfully');
      return { success: true };
    } catch (error) {
      return {
        success: false,
  ...
  async start(): Promise<{ success: boolean; message?: string; error?: string }> {
    ...
    if (state === 'starting' || state === 'running') {
      return { success: true, message: `Node-RED is already ${state}` };
    }

    // Guard checks
    if (state === 'disabled' || state === 'unavailable') {
      return { success: false, error: `Cannot start Node-RED in ${state} state` };
    }
    ...
    try {
      ...
      this.setState('failed');
      return { success: false, error: `Failed to write settings.js: ${error}` };
    }
    ...
    try {
      ...
      this.logger.info('Node-RED starting', { pid: proc.pid, port: this.config.port });

      return { success: true, message: 'Node-RED is starting' };
    } catch (error) {
      this.setState('failed');
      return { success: false, error: `Failed to spawn Node-RED: ${error}` };
    }
  }
    ...
    this.runtimeState.readinessPollTimer = setInterval(async () => {
      ...
      try {
        ...
        if (response.ok && response.status === 200) {
          ...
          this.startHealthCheckTimer();
          this.publishStatus('running', { port: this.config.port });
          this.logger.info('Node-RED ready', { port: this.config.port });
        }
      } catch {
      ...
      if (attempts >= maxAttempts) {
        ...
        this.publishStatus('failed', { error: 'Node-RED failed to start within 30 seconds' });
        this.logger.error('Node-RED failed to start within 30 seconds');
      }
    }, 500);
  }
  ...
  private async performHealthCheck(): Promise<void> {
    if (this.runtimeState.state !== 'running') return;
  ...
  private attachLogHandlers(proc: ReturnType<typeof Bun.spawn>): void {
    ...
    const logDir = this.paths.home('logs');
    if (!fs.existsSync(logDir)) {
      fs.mkdirSync(logDir, { recursive: true });
    }

    // Open log file handle for async append
    const logPath = this.paths.home('logs', 'nodered.log');
  ...
  private handleProcessExit(code: number): void {
    ...
    if (this.runtimeState.intentionalStop) {
      ...
      this.publishStatus('error', { error: `Node-RED process exited with code ${code}` });
      this.logger.warn('Node-RED crashed', { exitCode: code, restartCount: this.runtimeState.restartCount });
      ...
      if (this.runtimeState.restartCount <= this.config.maxRestartAttempts) {
        ...
        this.logger.error('Node-RED restart attempts exhausted', {
  ...
  async stop(): Promise<void> {
    ...
    this.runtimeState.process = null;
    this.publishStatus('stopped');
    this.logger.info('Node-RED stopped');
  }
