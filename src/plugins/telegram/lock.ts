...
export async function acquireLock(state: TelegramState): Promise<boolean> {
...
export async function releaseLock(state: TelegramState): Promise<void> {
...
export async function flushRuntimeFiles(state: TelegramState): Promise<void> {
  await releaseLock(state);

  const { configTmpPath, legacyChatStatePath, locksDirPath } = state.paths;
  const transientPaths = [configTmpPath, legacyChatStatePath];
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (!name.includes('telegram') || !name.endsWith('.lock')) continue;
