...
export class DiffPanel {
  ...
  constructor(renderer: CliRenderer) {
    ...
    const title = new TextRenderable(renderer, {
      id: 'diff-title',
      content: t`${bold(fg(theme.accent)('Diffs'))} ${dim(fg(theme.muted)('Ctrl+D to hide'))}`,
      height: 1,
    });
  ...
  private addDiffFallback(diffContent: string): void {
    ...
    for (const line of lines) {
      ...
      if (line.startsWith('+') && !line.startsWith('+++')) {
        content = t`${fg(theme.diffAddedFg)(line)}`;
      } else if (line.startsWith('-') && !line.startsWith('---')) {
        content = t`${fg(theme.diffRemovedFg)(line)}`;
      } else if (line.startsWith('@@')) {
        content = t`${fg(theme.accent)(line)}`;
      } else {
