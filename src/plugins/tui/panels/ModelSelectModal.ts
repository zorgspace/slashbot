 ...
 * Uses new palette colors for visual consistency.
 */
...
export class ModelSelectModal {
  ...
  constructor(renderer: CliRenderer) {
    ...
    const title = new TextRenderable(renderer, {
      id: 'model-title',
      content: t`${bold(fg(theme.accent)('Select AI Model'))}`,
      height: 1,
    });
  ...
  setModels(currentModel: string, entries: ModelEntry[]): void {
  ...
  private updateOptions(): void {
    const options: SelectOption[] = this.modelEntries.map(entry => ({
      name: entry.id === this.currentModel ? `${entry.name} (current)` : entry.name,
      description: `${entry.providerName}  -  ${entry.id}`,
      value: entry.id,
    }));
    this.select.options = options;

    // Pre-select current model
    const currentIndex = this.modelEntries.findIndex(e => e.id === this.currentModel);
    if (currentIndex >= 0) {
