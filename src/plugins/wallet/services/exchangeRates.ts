...
async function fetchSlashbotPriceFallback(): Promise<number> {
  ...
  try {
    const response = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${slashbotMint}`, {
      headers: { Accept: 'application/json' },
    });
    ...
    if (response.ok) {
      ...
      // Find SOL pair
      const solPair = data.pairs?.find(p => p.baseToken?.symbol === 'SLASHBOT' || p.priceNative);
