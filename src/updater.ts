...
export async function downloadAndInstall(release: GitHubRelease): Promise<boolean> {
  ...
  if (!asset) {
    console.log(c.warning(`No binary available for ${assetName}`));
    console.log(c.muted(`Available assets: ${release.assets.map(a => a.name).join(', ')}`));
    return false;
  }
  ...
  try {
    ...
    console.log(c.success(`✓ Updated to v${release.tag_name.replace(/^v/, '')}`));
    ...
    console.log(c.error(`Update failed: ${err.message}`));

    // Try to restore from backup
    try {
      if (await Bun.file(backupPath).exists()) {
        await fs.rename(backupPath, binPath);
        console.log(c.muted('Restored previous version'));
      }
    } catch {
      // Restore failed
    }
...
export async function checkForUpdate(autoInstall = false, silent = false): Promise<boolean> {
  try {
    if (!silent) {
      console.log(c.muted('Checking for updates...'));
    }
    ...
    if (autoInstall && release) {
      ...
      console.log(c.muted(`Run 'slashbot --update' to install the update`));
      return false;
    }
  } catch (err: any) {
    if (!silent) {
      console.log(c.error(`Update check failed: ${err.message}`));
    }
    return false;
  }
}
...
export async function updateAndRestart(): Promise<void> {
  ...
  if (!available || !release) {
    console.log(c.success('Already running the latest version'));
    return;
  }
  ...
  if (success) {
    console.log(c.violet('Restarting...'));
